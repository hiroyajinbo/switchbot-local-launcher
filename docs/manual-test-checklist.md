...
- [〇] 誤ったOpen TokenまたはSecret Keyは保存されず、入力画面にエラーが表示される
- [〇] 正しい認証情報を保存すると通常画面へ切り替わる
- [〇] 再起動後も入力不要でWindows資格情報から接続できる
- [〇] 通常画面の「API設定」で誤った認証情報を送信しても、現在の認証情報が上書きされない
- [〇] 通常画面の「API設定」で正しい認証情報へ更新し、通常画面で接続を継続できる
- [〇] 認証情報の更新後に再起動しても、再入力なしで接続を継続できる
- [〇] ポータブル版のEXEを直接起動しても同梱フォルダがデータ保存先になり、`config.json`がなければ自動作成される
- [〇] 初回設定後もポータブルフォルダとZIPにToken・Secretが生成されない
...
2026-07-23、通常のポータブルフォルダとZIPを再生成し、`.env`・`config.json`に加えて
開発用`.env`内のToken／Secret文字列そのものが成果物へ含まれないことを自動検査で確認。
2026-07-24、ポータブルZIPを検証用フォルダへ新規展開し、EXEの直接起動で
保存モードが`portable`、設定先が展開先直下、自動生成された`config.json`であることを確認。
2026-07-24、ポータブル版のEXE直接起動でも認証情報を初回登録でき、
通常画面の「API設定」から正しい認証情報へ再設定できることを確認。
2026-07-24、Windows資格情報からSwitchBot認証情報を削除し、再展開したポータブル版の
EXE直接起動で初回入力画面を確認。誤った認証情報は拒否され、正しい認証情報の保存後に
通常画面へ切り替わり、機器情報を取得できることを確認。
...
- [〇] 編集モードでクイック操作グループを追加し、シーンと赤外線クイック操作の選択肢へ表示される
- [〇] 「表示設定」で登録済みグループを選ぶと即時保存され、同名グループへ移動する
- [〇] グループの三点メニューから並べ替え・削除でき、削除したグループの操作は「シーン」へ移動する
- [〇] クイック操作の件数付きグループ見出しから折りたたみ・展開でき、再読み込み後も状態が維持される
- [〇] 編集モードの「削除・除外」ボタン文字が縦方向中央に表示される
...
- [〇] PCアプリを終了・再起動しても選択したライト／ダークモードが維持される
...
- [〇] デバイスカードの機器種別が日本語表示され、元のAPI種別はホバー時の補足として確認できる
- [〇] 横幅の広いカードでも明るさ・色温度スライダーが長くなりすぎず、値との対応が分かりやすい
...
- [〇] 起動時に候補が自動取得され、未追加シーンがある場合だけクイック操作上部へ件数が表示される
- [〇] 「すべて追加」で未追加シーンだけが即時にクイック操作へ追加される
- [〇] 「個別に選ぶ」から編集モードの候補一覧へ移動し、選択したシーンだけを追加できる
...
- [〇] 「新しいシーンを自動追加」を有効にすると未追加シーンが即時追加され、再起動後も設定が維持される
- [〇] 自動追加・削除・除外によって、デバイス設定と手動登録した赤外線クイック操作が変更されない
- [〇] SwitchBot側で取得できなくなった既存設定は自動削除されず、「取得できない設定」に残る
...
2026-07-24、再起動後も認証情報更新、クイック操作グループの追加・移動・並べ替え・
折りたたみ、シーンの取得・追加・除外・自動追加が維持されることを確認。
2026-07-24、内部ID衝突の修正後、起動時のシーン自動取得・自動追加により
SwitchBot側で追加した「扇風機イオン」が「シーン」グループへ表示されることを確認。
2026-07-24、実設定を変更しない検証用コピーとローカル画面を使い、未追加件数表示、
個別追加、グループの並べ替え・削除・既定グループへの退避、削除ボタン配置を確認。
2026-07-24、シーン同期の前後でデバイス設定・赤外線クイック操作を比較し、
取得不能設定も明示的に削除するまで保持されることを自動テストで確認。
